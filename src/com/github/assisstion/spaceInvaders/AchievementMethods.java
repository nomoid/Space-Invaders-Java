...
public final class AchievementMethods {
	...
	public static void checkName(String tempname) {
		...
		if (tempname.equalsIgnoreCase("god")
				|| tempname.equalsIgnoreCase("allah")
				|| tempname.equalsIgnoreCase("shiva")
				|| tempname.equalsIgnoreCase("mallah")) {
	...
	/*
	 * public static enum achievementList{ IGotsThePower, BunnyHop, SaveMeWallE,
	 * Inception, Tofuception, Mickception, Assisception,
	 * 
	 * Abstinence, SecondAmendment, ThisIsSparta, Untouchable, Marksman, Merida,
	 * AnnieOakley, Streaker, Streakaholic, StreakMaster,
	 * SomeKindOfSadisticMonster,
	 * 
	 * OhtheHugeManatee, OhtheUnhumanity, OhtheHumanity, BlindFaith,
	 * FernandoTorres,
	 * 
	 * 
	 * //things found in engine class Leeroy, Genocide, LanceArmstrong,
	 * TofuHull,
	 * 
	 * //things found elsewhere Dedication, Modesty,
	 * 
	 * }
	 */
	...
	public static void redeemAchievement(Achievement leAchievement) {
		...
		if (instance == null
				&& !achievementChecker.contains(leAchievement.name)
				|| !achievementChecker.contains(leAchievement.name)
				&& !instance.godmodeOn && !instance.bossOn) {
			...
			try {
				...
				System.out.println("Engine not yet created; Achievement still unlocked");
			} finally {
